...
interface TokenSelectorProps {
  ...
  onSelectToken: (token: Token, amount: string, distributionMethod: 'fixed' | 'total') => void;
...
export const TokenSelector: React.FC<TokenSelectorProps> = ({
  ...
  const [distributionMethod, setDistributionMethod] = React.useState<'fixed' | 'total'>('fixed');
  ...
  const validateAmount = (amount: string, token: Token) => {
    ...
    if (token.tokenId === "ERG") {
      const minAmount = distributionMethod === 'fixed' ? 0.001 : 0.001 * selectedTokens.length;
      if (numAmount < minAmount) return `Minimum amount is ${minAmount} ERG`;
    ...
    const minAmount = distributionMethod === 'fixed' 
      ? 1 / Math.pow(10, decimals)
      : (1 / Math.pow(10, decimals)) * selectedTokens.length;
  ...
  const handleAddToken = () => {
    if (selectedToken && tokenAmount && !amountError) {
      ...
      onSelectToken(selectedToken, amount, distributionMethod);
      setSelectedToken(null);
      setTokenAmount("");
      setAmountError(null);
      setDistributionMethod('fixed'); // Reset to default
    }
  };
    ...
    <div className="space-y-4">
      <div className="bg-white/30 backdrop-blur-sm p-4 rounded-lg pixel-borders">
        ...
        <div className="space-y-4">
          ...
          >
            <SelectTrigger className="bg-white border-gray-200 text-ocean-dark">
          ...
          <div className="flex gap-2">
            <Select
              ...
              onValueChange={(value: 'fixed' | 'total') => setDistributionMethod(value)}
            >
              <SelectTrigger className="bg-white border-gray-200 text-ocean-dark">
                <SelectValue placeholder="Distribution method" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="fixed">Fixed Amount per Recipient</SelectItem>
                <SelectItem value="total">Total Amount to Distribute</SelectItem>
              </SelectContent>
            </Select>
            ...
            <Input
              ...
              placeholder={selectedToken ? `${distributionMethod === 'fixed' ? 'Amount per recipient' : 'Total amount'} (min ${
      ...
      {selectedTokens.length > 0 && (
        <div className="bg-white/30 backdrop-blur-sm p-4 rounded-lg pixel-borders space-y-2">
          ...
          {selectedTokens.map(({ token, amount, distributionMethod }) => (
            <div key={token.tokenId} className="flex items-center justify-between p-2 bg-white/50 rounded">
              <span className="text-ocean-dark font-medium">
                {token.name || 'Unnamed Token'}: {
                  token.tokenId === "ERG" 
                    ? `${(Number(amount) / 1000000000).toFixed(4)} ERG ${distributionMethod === 'total' ? '(total)' : '(per recipient)'}`
                    : `${token.decimals 
                        ? (Number(amount) / Math.pow(10, token.decimals)).toFixed(Math.min(token.decimals, 4))
                        : amount
                      } ${distributionMethod === 'total' ? '(total)' : '(per recipient)'}`
                }
              </span>
              <Button
                variant="ghost"
